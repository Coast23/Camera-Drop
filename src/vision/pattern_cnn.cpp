...
cv::Mat normalize_gray_input(const cv::Mat& input) {
    cv::Mat gray;
    if (input.channels() == 1) {
        gray = input;
        ...
        throw std::runtime_error("unsupported deskewed image format for pattern cnn");
    }
    if (gray.cols == kImageWidth && gray.rows == kImageHeight) {
        return gray;
    }
    cv::Mat resized;
    cv::resize(gray, resized, cv::Size(kImageWidth, kImageHeight), 0.0, 0.0, cv::INTER_LINEAR);
    return resized;
}
...
PatternCnnClassifier::PatternCnnClassifier(const std::string& model_path, PatternCnnOptions options)
    : options_(std::move(options)),
      env_(ORT_LOGGING_LEVEL_WARNING, "pattern_cnn"),
    ...
    if (payload_x_.size() != Config::PAYLOAD_SYMBOL_COUNT) {
        throw std::runtime_error("pattern cnn payload position count mismatch");
    }
}

std::vector<uint8_t> PatternCnnClassifier::PredictPayloadPatterns(const cv::Mat& deskewed) {
    if (deskewed.empty()) {
        return {};
    }

    const cv::Mat gray = normalize_gray_input(deskewed);
    const size_t sample_count = payload_x_.size();
    std::vector<uint8_t> predictions(sample_count, 0);
    std::vector<float> input_buffer(std::min(sample_count, options_.batch_size) * kPatchArea, 0.0f);
    ...
    for (size_t begin = 0; begin < sample_count; begin += batch_limit) {
        ...
        if (outputs.empty() || !outputs[0].IsTensor()) {
            throw std::runtime_error("pattern cnn returned empty output");
        }
        const auto info = outputs[0].GetTensorTypeAndShapeInfo();
        const size_t total_logits = info.GetElementCount();
        if (total_logits != batch * kNumClasses) {
            throw std::runtime_error("pattern cnn output shape mismatch");
        }
